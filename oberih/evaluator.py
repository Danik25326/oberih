...
- Result<T,E> — справжній OberihResult тип (не dict, не exception).
  Оператор "?" розпаковує Ok або пробрасує Err через PropagateErr.
"""

from lark import Tree, Token
from interpreter import call_resilient, network, _timeout_executor
from oberih_result import OberihResult, PropagateErr, OberihUnwrapError
    ...
    if isinstance(node, Tree) and node.data == "primary":
        ...
        for pf in postfixes[:-1]:
            if pf.data == "field_access":
                ...
                current = current.get_field(field) if isinstance(current, OberihStruct) else current[field]
        ...
        if last_pf.data == "field_access":
            field = str(last_pf.children[0])
            if isinstance(current, OberihStruct):
    ...
    if bytecode is not None:
        ...
        def body_fn(*call_args):
            ...
            try:
                ...
                # ? оператор підняв Err — повертаємо його як Result цієї функції
                return e.result
...
def _format_value(v):
    ...
    if isinstance(v, OberihResult):
        return repr(v)
    if isinstance(v, OberihStruct):
        fields = object.__getattribute__(v, "_fields")
        raw = fields.get("__fields__")
        if raw is not None:
            parts = ", ".join(_format_value(f) for f in raw)
            return f'{v.type_name}({parts})'
        field_strs = ", ".join(
            f"{k}: {_format_value(val)}" for k, val in fields.items()
        )
        return f'{v.type_name}{{{field_strs}}}'
...
def call_spawn_handle_method(handle: OberihSpawnHandle, method_name, args):
    if method_name == "join":
        timeout = args[0] if args else None
        return handle.join(timeout=timeout)
    if method_name == "cancel":
        return handle.cancel()
    if method_name == "isDone":
        return handle.is_done()
    if method_name == "isCancelled":
...
def call_by_name(name, args, env):
    ...
    if name == "Ok":
        return OberihResult.ok(args[0] if args else None)
    if name == "Err":
        return OberihResult.err(args[0] if args else None)
    ...
    if name in STRUCTS:
        ...
        return OberihStruct(name, fields)
    # Невідома функція з великої літери -> генерична структура без означення
    return OberihStruct(name, {"__fields__": args})
...
def eval_expr(node, env):
    ...
    if node.data == "spawn_expr":
        ...
        return OberihSpawnHandle(future, token, fn_name)
    ...
    if node.data == "match_expr":
        ...
        for arm in node.children[1:]:
            ...
            if pattern_node.data == "ctor_pattern":
                ...
                if isinstance(scrutinee, OberihResult) and scrutinee.variant == ctor_name:
    ...
    if node.data == "try_op":
        val = eval_expr(node.children[0], env)
        if isinstance(val, OberihResult):
            if val.is_ok():
                return val.unwrap()   # розпаковуємо Ok — повертаємо внутрішнє значення
            else:
                raise PropagateErr(val)  # Err — пробрасуємо вгору
        # Якщо значення не Result — пропускаємо (сумісність зі старим кодом)
        return val
...
def eval_primary(node, env):
    ...
    for pf in postfixes:
        if pf.data == "call_args":
            ...
            elif isinstance(current, OberihStruct):
            ...
            if isinstance(current, NameRef):
                ...
                isinstance(current, OberihStruct)
                and f"{current.type_name}.{method}" in REGISTRY
            ):
                current = call_by_name(f"{current.type_name}.{method}", [current] + list(args), env)
            elif isinstance(current, (list, dict)):
                current = call_container_method(current, method, args)
            elif isinstance(current, OberihSpawnHandle):
