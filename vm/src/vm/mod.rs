...
impl VM {
        ...
        while ip < code.len() {
            ...
            match instr {
                ...
                Instr::LoadGlobal(name) => {
                    // Перевіряємо чи це функція в модулі
                    if self.module.find_fn(name).is_some() {
                        push!(Value::Fn(name.clone()));
                    } else {
                        push!(self.load_builtin(name)?);
                    }
                }
                ...
                Instr::MakeStruct { name, field_count } => {
                    // Стек: [field_N ... field_1 field_names...]
                    // Поки що беремо N значень без імен — VM потребує метадані struct
                    // TODO: передавати імена через compile-time таблицю
                    let mut fields = HashMap::new();
                    let vals: Vec<Value> = (0..*field_count).map(|_| stack.pop().unwrap_or(Value::Nil)).collect();
                    for (i, v) in vals.into_iter().rev().enumerate() {
                        fields.insert(format!("_f{}", i), v);
                    }
    ...
    fn load_builtin(&self, name: &str) -> VR<Value> {
        match name {
            "print" | "println" | "len" | "toString" | "toNumber" => {
                Ok(Value::Fn(format!("__builtin_{}", name)))
            }
            _ => Err(rt_err(format!("Невідоме ім'я: '{}'", name))),
        }
    }

    pub fn call_builtin(&self, name: &str, args: Vec<Value>) -> VR<Value> {
        match name {
            "__builtin_print" => {
            ...
            "__builtin_len" => {
                match args.into_iter().next() {
                    ...
                    _ => Err(rt_err("len() потребує List або String")),
                }
            }
            "__builtin_toString" => {
            ...
            "__builtin_toNumber" => {
                match args.into_iter().next() {
                    Some(Value::Str(s)) => s.parse::<f64>()
                        .map(Value::Num)
                        .map_err(|_| rt_err(format!("Не можна перетворити '{}' в число", s))),
                    Some(Value::Num(n)) => Ok(Value::Num(n)),
                    _ => Err(rt_err("toNumber() потребує String або Number")),
                }
            }
